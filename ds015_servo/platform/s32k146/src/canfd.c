...
/*!
 * @brief Bit field declaration for the transmission and reception message buffers. See "Message Buffer Structure" in
 * RM. NOTE: Since this example manages CAN FD (8 bytes for header and 64 bytes for payload) you have 7 Message Buffers
 * available. The FIFO is not enabled.
 */
typedef struct
{
    struct
...
/*!
 * @brief Cast the structure above to the CAN0 memory area. You can go to the "S32K146_bitfields.h" file and see this
 * memory area. The file is inside the project's include folder. CAN0_BASE + 0x80 is the specific address where the CAN0
 * MB structure is defined.
 */
...
FlexCAN_bit_timings_t CANFD_bitrate_profile_LUT[CANFD_bitrate_profile_NUM] = {{.EPRESDIV = 0,
                                                                               .EPROPSEG = 0,
                                                                               .EPSEG1   = 0,
                                                                               .EPSEG2   = 0,
                                                                               .ERJW     = 0,
...
status_t FlexCAN0_Init(CANFD_bitrate_profile_t profile, uint8_t irq_priority, void (*callback)())
{
    ...
    /* CAN Bit Timing (CBT) configuration for a nominal phase of 1 Mbit/s with 80 time quantas, in accordance with Bosch
     * 2012 specification, sample point at 83.75% */
    ...
    /* CAN-FD Bit Timing (FDCBT) for a data phase of 4 Mbit/s with 20 time quantas, in accordance with Bosch 2012
     * specification, sample point at 75% */
...
status_t FlexCAN0_Install_ID(uint32_t id, uint8_t mb_index)
{
    ...
    CAN0->CAN0_RXIMR[mb_index] = BIT_SRV_NOT_MSG(1) | BIT_R23(1) | (0x1FFF << 8) | BIT_MSG_R7(1);
    ...
    CAN0_MB->FD_MessageBuffer[mb_index].CODE =
        4; /* When a frame is received successfully, this field is automatically updated to FULL */
    ...
    CAN0_MB->FD_MessageBuffer[mb_index].EXT_ID =
        (BIT_SRV_NOT_MSG(0) | BIT_R23(0) | (id << 8) | BIT_MSG_R7(0)) & 0x1FFFFFFF;
