...
///                             /_/                     /____/_/
///
/// This module implements non-volatile key-value storage for this application. On an embedded system this may be
/// backed by a trivial key-value list stored in a memory chip, or perhaps a compact fault-tolerant file system,
/// like LittleFS. High-integrity embedded systems may benefit from not accessing the storage memory during
/// normal operation at all; the recommended approach is to read the configuration from the storage memory
/// once during the boot-up and then keep it in RAM; if new configuration needs to be stored, it is to be done
...
///
/// The function is blocking and the execution time is not rigidly bounded, so it should only be invoked
/// during the boot-up or before the reboot.
